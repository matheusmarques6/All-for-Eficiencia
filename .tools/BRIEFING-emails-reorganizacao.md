...
Data: 2026-09-09. Origem: `admin-convertfy/docs/email-generation/diagnostico-vault-vs-advisor-max.md`.
Este arquivo vive em `.tools/` de propósito: fora de `Emails/`, o sincronizador não o lê.
As seções 1–4 são o briefing original, verbatim. A seção 5 são as emendas do executor,
decididas antes da execução — valem como parte do briefing.
...
O resumo do diagnóstico: o vault está certo; o que está errado é a
...
seção 4, fora do escopo de quem edita o vault.
...
**Por quê:** o Max tem uma persona que entra em toda resposta e
...
Pulada nesta execução (decisão do executor: só depois de a telemetria
produzir o primeiro rascunho de lacuna).
...
Nada abaixo é de quem edita o vault. Está aqui para saber o que vem
depois e por onde medir.

### 4.3 As medidas

| Medida | Hoje | Alvo | Depende de |
|---|---|---|---|
| `lacunas_servidas` por run do Curador | 0 | > 0 quando a seção tem lacuna | T3 + código |
...
## 5. Emendas do executor (2026-09-09) — valem como parte do briefing

Decididas depois de verificar o vault real. Estado verificado antes de
começar: `Conhecimento/Advisors/Max/` existe com os 11 caminhos do T7;
`intencoes/` só tem `welcome`; 0/8 intenções têm `modo:`; 0/18
aprendizados têm `serve_a:`; 91 notas do escopo de T6 começam com `#`;
`componentes/doutrina/` e `_julgamento.md` não existem.

**E1 — T1 entra como está escrito, e a regra 5 vira lacuna registrada.**
Os 8 vocabulários novos (~40 valores) não têm nota em `eixos/`. Isso não
quebra `valida.py` (ele só valida `variantes/`), mas contraria o espírito
da regra 5. Não se cria as ~40 notas agora. Em vez disso, T1 também cria
`componentes/lacunas/vocabulario-do-contrato-sem-nota-de-eixo.md`
(`status: aberta`) listando os 8 campos, os valores e o que o agente perde
por não ter definição legível. Decisão de criar as notas fica para depois.

**E2 — T7: a ponte herda o `registro:` da origem.** Toda nota de ponte
carrega `registro:` copiado da nota do Max que a origina, e `fonte:` cita
o caminho **e a(s) linha(s)** da nota do Max de onde a regra sai (não só a
pasta). Onde a origem tiver `outro-narrador` — verificado:
`copy/subject-lines.md` e
`doutrina/otimize-para-a-varredura-nao-para-a-leitura.md` — a ponte abre
com `## Aviso de autoria` dizendo que parte da regra não é citável como
fala do Max. O briefing avisava só a primeira; são duas.

**E3 — T8: cada intenção derivada declara de onde veio e onde a origem
cala.** `fonte:` com o caminho da nota do Max em toda intenção derivada
dele. O `_flow.md` de cada flow ganha uma seção `# Onde a origem cala` —
o que foi escrito sem lastro no Max e precisa de revisão humana com mais
cuidado. Já sabido: cart/checkout no Max tem 4 emails e **zero delay,
zero filtro, zero condição de saída**; o produto tem 8 toques. Metade vai
ser escrita sem origem — isso tem que estar visível para quem revisar.
`shipping_stages` é inteiro sem origem.

**E4 — T6: a regra nova de `valida.py` tem escopo explícito.** Aplica-se
só a `componentes/variantes/`, `componentes/secoes/`,
`componentes/convivencia/`, `componentes/lacunas/`, `estruturas/` e
`aprendizados/`. **Não** se aplica a `componentes/eixos/`,
`componentes/requisitos/` nem aos arquivos `componentes/*.md` da raiz
(que começam com `# Título` de propósito). Sem o escopo, a regra reprova
30+ notas que estão certas.

**E5 — nomes.** Os casos de teste do Max são `C-01`…`C-25`, não "A e B".

**E6 — ordem de execução e colisão de arquivos.** Onda 1 em paralelo: T1,
T2, T3, T5 (arquivos disjuntos). Onda 2, depois que a onda 1 fechar: T4,
T6, T7, T8 (T6 toca aprendizados/lacunas/variantes que T5/T3/T2 editaram —
por isso só depois; T4 lê as `proibicoes` que T1 escreveu). Cada tarefa
tem executor e revisor; o revisor audita contra o vault e contra este
briefing, corrige erro objetivo, relata julgamento.
