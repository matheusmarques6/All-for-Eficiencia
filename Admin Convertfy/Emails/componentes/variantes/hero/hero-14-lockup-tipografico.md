...
## Descrição curta
Transforma a oferta em tipografia monumental: o leitor lê o valor antes de qualquer frase, num lockup que ocupa um terço da peça — e NÃO recebe código, porque esta hero declara a oferta sem entregá-la. Obriga sobrescrito, palavra de reforço repetida, valor, sufixo, título de coleção, corpo, CTA, imagem do lockup, fundo e logo.

## Descrição detalhada
Logo no topo (`lockup_logo_image`), sobrescrito curto (`lockup_eyebrow`), lockup tipográfico que ocupa um terço da altura — a palavra de reforço repetida (`lockup_repeat_word`) emoldurando o valor (`lockup_value`) e o sufixo (`lockup_suffix`) — apoiado em `lockup_type_image` (arte tipográfica gerada), seguido de título de coleção (`lockup_collection_title`), corpo (`lockup_body`) e CTA (`lockup_cta_label`), sobre `lockup_background_image`. NÃO há campo de cupom: é a diferença estrutural para a [[hero-11-bogo-em-manchete]] (mesmo dispositivo, código em campo próprio). Das campanhas nomeadas (13, 15) difere por não citar data: a manchete é o valor, não a ocasião.
...
## Notas de cadastro
Descrição do banco bate com a anatomia (sem campo de cupom, confirmado nos 10 campos). Sem HTML para medir altura — `peso` omitido; ver relatório.
