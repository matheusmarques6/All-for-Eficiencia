...
`doutrina-segundos-de-atencao` está em [[#Conflitos entre módulos]].
...
Absorve `sms-delay-do-popup` e `fundamentos-time-delay-do-form`. **Sete
formulações.**
...
está em [[#Conflitos entre módulos]].
...
| Recap final — welcome → "maybe three filler emails" → absolute last chance → "is everything okay?" | 6 | transcricao | L2308-2318 |
...
o denominador explícito. Conflito irmão: `fundamentos-denominador-dos-80`.
...
`copy-janela-de-atencao` está em [[#Conflitos entre módulos]] (dentro de
`doutrina-segundos-de-atencao`). `copy-numeracao-dos-principios` e
`copy-takeaways-por-email` estão em [[#Conflitos dentro do mesmo registro]].
...
`design-html-vs-imagem` está em [[#Conflitos entre módulos]];
`design-cta-por-produto` em [[#Conflitos dentro do mesmo registro]];
...
`flows-onde-testar` ([[#Conflitos entre módulos]]);
...
Ver também [[#Conflitos entre módulos]], onde a frequência de campanha aparece com
